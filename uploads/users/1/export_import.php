...
//			$worksheet->getStyle( $range )->applyFromArray( $style, false );
    ...
    protected function getOrders( &$languages, $exist_meta_title, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
//        var_dump($min_id);
//        var_dump($max_id);
//        die;
        $sql  = "SELECT c.* FROM `".DB_PREFIX."order` c ";
        if (isset($min_id) && isset($max_id)) {
            $sql .= "WHERE c.order_id BETWEEN $min_id AND $max_id ";
        }
    ...
    protected function populateCategoriesWorksheet( &$worksheet, &$languages, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
        ...
        $j = 0;
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('category_id'));
    ...
    protected function populateOrdersWorksheet (&$worksheet, &$languages, &$box_format, &$text_format, $offset = null, $rows = null, &$min_id = null, &$max_id = null) {
        $rowIndex = 1;
        $headerRow = [];
        $orderRow = [];
        $separatorRow = [];
        ...
        $headerRow[2] = "№ заказа на сайте";
        $worksheet -> getColumnDimension('B') -> setWidth(6);
        //$worksheet -> getColumnDimension('B') -> setWidth(6);
        $headerRow[3] = "Нужно ли подтверждение";
        $worksheet -> getColumnDimension('C') -> setWidth(4);
        $headerRow[4] = "Имя клиента";
        ...
        $headerRow[7] = "Дата создания";
        $worksheet -> getColumnDimension('G') -> setWidth(15);
        $headerRow[8] = "Дата получения";
        $worksheet -> getColumnDimension('H') -> setWidth(15);
        $headerRow[9] = "Наименование товара";
        $worksheet -> getColumnDimension('I') -> setWidth(35);
        //$worksheet -> getColumnDimension('I') -> setAutoSize(true);
        $headerRow[10] = "Количество пирожных";
        $worksheet -> getColumnDimension('J') -> setWidth(4);
        $headerRow[11] = "Вкус";
        $worksheet -> getColumnDimension('K') -> setWidth(max(strlen('shipping_address_1'),19)+1);
        //$worksheet -> getColumnDimension('K') -> setAutoSize(true);
        $headerRow[12] = "Изменение надписи на открытке";
        $worksheet -> getColumnDimension('L') -> setWidth(max(strlen('custom_field'),19)+1);
        //$worksheet -> getColumnDimension('L') -> setAutoSize(true);
        $headerRow[13] = "Текст бумажной открытки";
        $worksheet -> getColumnDimension('M') -> setWidth(max(strlen('shipping_method'),16)+5);
        //$worksheet -> getColumnDimension('M') -> setAutoSize(true);
        $headerRow[14] = "Комментарий к заказу";
        $worksheet -> getColumnDimension('N') -> setWidth(max(strlen('order_status_id'),16)+5);
        //$worksheet -> getColumnDimension('N') -> setAutoSize(true);
        $headerRow[15] = "Доставка/самовывоз";
        //$worksheet -> getColumnDimensionByColumn(15) -> setWidth(15);
        //$worksheet -> getColumnDimension('O') -> setAutoSize(true);
        $headerRow[16] = "Интервал доставки";
        //$worksheet -> getColumnDimensionByColumn(16) -> setWidth(5);
        $headerRow[17] = "Адрес доставки";
        //$worksheet -> getColumnDimensionByColumn(17) -> setWidth(10);
        //$worksheet -> getColumnDimension('Q') -> setAutoSize(true);
        $headerRow[18] = "Имя для доставки";
        //$worksheet -> getColumnDimensionByColumn(18) -> setWidth(10);
        //$worksheet -> getColumnDimension('R') -> setAutoSize(true);
        $headerRow[19] = "Телефон для доставки";
        //$worksheet -> getColumnDimensionByColumn(19) -> setWidth(10);
        //$worksheet -> getColumnDimension('S') -> setAutoSize(true);
        $headerRow[20] = "Откуда узнали";
        //$worksheet -> getColumnDimensionByColumn(20) -> setWidth(10);
        //$worksheet -> getColumnDimension('T') -> setAutoSize(true);
        $headerRow[21] = "Курьер";
        //$worksheet -> getColumnDimension('U') -> setWidth(max(strlen('shipping_method'),16)+5);
        //$worksheet -> getColumnDimension('U') -> setAutoSize(true);
        $headerRow[22] = "Выручка";
        //$worksheet -> getColumnDimensionByColumn(22) -> setWidth(max(strlen('order_status_id'),16)+5);




        $worksheet -> getRowDimension($rowIndex) -> setRowHeight(90);
        $this -> setCellRow($worksheet, $rowIndex, $headerRow, $box_format);
        $rowIndex++;



        $orders = $this -> getOrders($languages, false, $offset, $rows, $min_id, $max_id);
        $min_id = $orders[0]['order_id'];
        $max_id = $orders[count($orders)-1]['order_id'];

        foreach ($orders as $order) {
            $confirmationNeeded = "";
            $dateAdded = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($order['date_added'], 0, 10))),date('m',strtotime(substr($order['date_added'], 0, 10))),date('d',strtotime(substr($order['date_added'], 0, 10))));//strftime('%d.%b', strtotime($order["date_added"]));
            $shippingAddress = "";
            $shippingDate = "";
            $profit = "";
            $howFound = "";
            ...
            $shipping = explode("\n", htmlspecialchars($order["shipping_address_1"]));
            // Адрес доставки
            $shippingAddress = $shipping[0] != " " ? $shipping[0] : "";
            // Дата доставки
            $date_for = "";
            if (isset($shipping[2])) {
                if ($shipping[2] === " ") {
            ...
            //if(isset($date_for)) {
                $qwee = date("d.M", strtotime(trim($date_for))); //data izmeneniya
                if ($qwee == '01.01.1970'){
                    $shippingDate = " ";
                } else {
                    $shippingDate = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(trim($date_for))),date('m',strtotime(trim($date_for))),date('d',strtotime(trim($date_for))));
                }
            //} else {
            //    $shippingDate = " ";
            //}
            ...
            //        $shippingDate = "";
            //    else
            //        $shippingDate = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($shipping[2], 0, 10))),date('m',strtotime(substr($shipping[2], 0, 10))),date('d',strtotime(substr($shipping[2], 0, 10))));
                //$shippingDate = $shipping[2] != " " ? strftime('%d.%b', strtotime($shipping[2])) : "";
                //$shippingDate = $shipping[2] != " " ? getdate(strtotime($shipping[2])) : "";
                //$qwee = date("d.M", strtotime(trim($shipping[2]))); //data izmeneniya
            //if ($qwee == '01.01.1970'){
            //    $shippingDate = "  ";
            //} else {
            //    $shippingDate  = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($order['date_added'], 0, 10))),date('m',strtotime(substr($order['date_added'], 0, 10))),date('d',strtotime(substr($order['date_added'], 0, 10))));
            //}

            $dateAdded = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($order['date_added'], 0, 10))),date('m',strtotime(substr($order['date_added'], 0, 10))),date('d',strtotime(substr($order['date_added'], 0, 10))));

            // Выручка
            //$profit = $order["payment_method"] == "ROBOKASSA" && intval($order["order_status_id"]) == 5 ? 0 : $order["total"];

            if ($order["payment_method"] == "ROBOKASSA") {
                $profit = intval($order["order_status_id"]) == 5 ? 0 : $order["total"];
            } else {
                $profit = $order["total"];
            }

            $order_coupon = $this -> db -> query("SELECT title FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $order['order_id'] . "'");
            $order_coupon = (array)$order_coupon;
            for ($n = 0; $n < count($order_coupon['rows']); $n++) {  //выгружаем купон
                if (strpos($order_coupon['rows'][$n]['title'], "Купон") === false) {
                    $coupon = " ";
                } else {
                    $coupon = $order_coupon["rows"][$n]['title'];
                    break;
                }
            }
            ...
            // Откуда узнали
            $howFound = $this -> db -> query("SELECT custom_field FROM " . DB_PREFIX . "customer WHERE email = '" . $order['email'] . "'");
            $howFound = (array)$howFound;
            if (isset($howFound['row']['custom_field'])) {
                $howFound = $howFound['row']['custom_field'];
            } else {
                $howFound = "";
            }

            if (isset($order['payment_firstname'])) {
                $shippingName = $order['payment_firstname'];
            } else {
                $shippingName = "";
            }

            if (isset($order['payment_lastname'])) {
                $shippingPhone = " " . $order['payment_lastname'];
            } else {
                $shippingPhone = ""; //nazvanie produkta
            }

            if(isset($order['comment'])){
                $addPresent = explode("\n",$order['comment']);

                for($m = 0; $m < count($addPresent); $m++) {
                    if (strpos($addPresent[$m], "!Необходимо") === false) {
                        $confirmationNeeded = "";
                    } else {
                        $confirmationNeeded = "да";
                        break;
                    }
                }

                            for($m = 0; $m < count($addPresent); $m++) {
                                if (strpos($addPresent[$m], "Текст") === false) {
                                    $cardText = "";
                                } else {
                                    $cardText = substr($addPresent[$m], (strpos($addPresent[$m], ":") + 1));
                                    break;
                                }
                            }

                            for($m = 0; $m < count($addPresent); $m++){
                                if (strpos($addPresent[$m],"Комментарий к заказу") === false) {
                                    $commentText = "";
                                } else {
                                    $comstart = $m;
                                    if(count($addPresent) > $comstart){
                                        $commentText = substr($addPresent[$m], (strpos($addPresent[$m], ":") + 1));
                                        for($commentadd = 1; $commentadd < (count($addPresent) - $comstart); $commentadd++){
                                            $commentText.=" ". $addPresent[$comstart+$commentadd];
                                        }
                                    } else {
                                        $commentText = substr($addPresent[$m], (strpos($addPresent[$m], ":") + 1));
                                    }
            ...
            $products = $this -> db -> query("SELECT * FROM ".DB_PREFIX."order_product WHERE order_id = '".$order['order_id']."'");
            $products = (array)$products;

            foreach ($products["rows"] as $productKey => $product) {
                $order_product_id = $product["order_product_id"];
                $product_id = $product["product_id"];

                $product_count = $this -> db -> query("SELECT isbn, upc FROM ".DB_PREFIX."product WHERE product_id = '".$product_id."'");
                $product_count = (array)$product_count;
                if($product_count['row']['upc']) {
                    $text_to_cart_add = $product_count['row']['upc']; // текст, добавляемый к открытке
                } else {
                    $text_to_cart_add = "";
                }
                ...
                $product_options = $this -> db -> query("SELECT * FROM ".DB_PREFIX."order_option WHERE order_id = '".$order['order_id']."' AND order_product_id = '".$product["order_product_id"]."'");
                $options = (array)$product_options;

                //$order_product_id = $this -> db -> query("SELECT order_product_id FROM " . DB_PREFIX . "order_product WHERE order_id = '" . $order['order_id'] . "'");
                //$order_product_id = (array)$order_product_id;
                //$order_product_id = $order_product_id['row']['order_product_id'];
                ...
                //if (count($options['rows']) >= 0) {
                    for ($u = 0; $u < count($options['rows']); $u++) {

                        if (trim($options['rows'][$u]['name']) == "Приложить открытку") {
                            $cart_with = "";
                            if (strpos($options['rows'][$u]['value'], "@%")) {
                                $cart_with = explode("@%", $options['rows'][$u]['value']);
                            }
                        }

                        if ($options['rows'][$u]['name'] == "Введите имя") {
                            $newCartText = $text_to_cart_add.' '.(string)$options['rows'][$u]['value'];
                        }

                        if ($options['rows'][$u]['name'] == "Введите надпись ") {
                            $newCartText = (string)$options['rows'][$u]['value'];
                        }

                        if ($options['rows'][$u]['name'] == "Текст открытки") {
                            $paperCardText = (string)$options['rows'][$u]['value'];
                        }

                        if ($order['custom_field']) {
                            $inter = substr($order['custom_field'], strpos($order['custom_field'], '"'), 7);
                            if (strpos($order['custom_field'], "empty")) {
                                $shippingTime = "";
                            } elseif (strpos($inter, "11-14")) {
                                $shippingTime = '11-14';
                            } elseif (strpos($inter, "14-17")) {
                                $shippingTime = "14-17";
                            } elseif (strpos($inter, "17-20")) {
                                $shippingTime = "17-20";
                            } else {
                                $shippingTime = substr($order["custom_field"], 6, -2);
                            }
                        } else {
                            $shippingTime = ""; //interval dostavki pustoi
                        }


                        if ($options['rows'][$u]['name'] == "Вкус количество") {
                            if (trim($options['rows'][$u]['value']) == "@%") {
                                //if (isset($tastes['rows'][$u-1]['value'])) {
                                //ToDo Возможно вкус не только в 0 ячейке
                                if (isset($options['rows'][0]['value'])) {
                                    if ($options['rows'][$u]['order_product_id'] == $options['rows'][0]['order_product_id']) {
                                        $tasty[0][0] = $options['rows'][0]['value'];
                                        if ($options['rows'][$u]['name'] !== 'Текст открытки' && $options['rows'][$u]['name'] !== 'Дополнительно:') {
                                            if ($options['rows'][$u]['value'] == '@%') {
                                                for ($re = 0; $re < count($options['rows']); $re++) {
                                                    if ($options['rows'][$re]['name'] == "Вкус единичный") {
                                                        $tasty[0][0] = $options['rows'][$re]['value'];
                                                    }
                                                }
                                                //$tasty[0][0]='ассорти';
                                            } else
                                                $tasty[0][0] = $options['rows'][$u]['value'];
                                            }
                                        } else
                                            $tasty[0][0] = $options['rows'][$u]['value'];
                                    } else
                                        $tasty[0][0] = $tastes['rows'][$u+1]['value'];


                                    if (isset($product_count)) {
                                        $tasty[0][1] = $product_count;
                                    } else {
                                        $tasty[0][1] = "";
                                    }
                                } elseif (strpos($options['rows'][$u]['value'], ";")) {
                                    $taste = explode(";", $options['rows'][$u]['value']);
                                    $tasty1 = array_pop($taste);
                                    unset($tasty);
                                    $tasty = [];
                                    foreach ($taste as $tasted) {
                                        $tasty[] = explode(":", $tasted);
                                    }
                                } elseif((trim($options['rows'][$u]['value']) === "@%") && (count($options['rows']) == 2) && (isset( $options['rows'][$u+1]))){
                                    $tasty[0][0] = $options['rows'][$u-1]['value'];
                                } else {
                                    $tasty[0][0] = "";
                                    $tasty[0][1] = "";
                                }
                ...
                } elseif (count($options['rows']) == 1) {
                    for ($u = 0; $u < count($options['rows']); $u++) {
                        if ($options['rows'][$u]['name'] == "Вкус количество") {
                            if (trim($options['rows'][$u]['value']) == ";@%") {
                                if ($options['rows'][$u]['order_product_id'] == $options['rows'][$u - 1]['order_product_id'])
                                    $tasty[0][0] = $options['rows'][$u - 1]['value'];
                                else
                                    $tasty[0][0] = " ";
                                ...
                                if($options['rows'][$u]['name'] !== "Вкус единичный") {
                                    $tasty[0][0] = "ассорти";
                                    $tasty[0][1] = $product_count;
                                }
                            }
                ...
                $quantity = $product["quantity"];
                $cakes_count = $product_count['row']["isbn"]; //количество пирожных в 1 коробке

                if (count($products["rows"]) == 1) {
                    if ($quantity > 1) {
                        for ($q = 1; $q <= $quantity; $q++) {
                            if ($q == 1) {
                                if (count($tasty) >= 1) {
                                    ...
                                    while($s < count($tasty)) {
                                        $tasteRow[1] = "";
                                        $tasteRow[2] = $s == 0 ? $order["order_id"] : "";
                                        $tasteRow[3] = $s == 0 ? $confirmationNeeded : "";
                                        $tasteRow[4] = $s == 0 ? $order["firstname"] : "";
                                        $tasteRow[5] = $s == 0 ? $order["telephone"] : "";
                                        $tasteRow[6] = $s == 0 ? $order["email"] : "";
                                        $tasteRow[7] = $dateAdded;
                                        $tasteRow[8] = $shippingDate;
                                        $tasteRow[9] = $s == 0 ? $product["name"] : "";
                                        $tasteRow[10] = (int)$tasty[($s)][1];//count($tasty) > 1 ? (int)$tasty[$s][1] : (int)$tasty[$s][0];
                                        $tasteRow[11] = $tasty[$s][0];
                                        $tasteRow[12] = $s == 0 ? $newCartText : "";
                                        $tasteRow[13] = $s == 0 ? $paperCardText : "";
                                        $tasteRow[14] = $s == 0 ? $commentText : "";
                                        $tasteRow[15] = $order["shipping_method"];
                                        $tasteRow[16] = $shippingTime;
                                        $tasteRow[17] = $s == 0 ? $shippingAddress : "";
                                        $tasteRow[18] = $s == 0 ? $shippingName : "";;
                                        $tasteRow[19] = $s == 0 ? $shippingPhone : "";
                                        $tasteRow[20] = $s == 0 ? $howFound : "";
                                        $tasteRow[21] = $s == 0 ? $coupon : "";
                                        $tasteRow[22] = $s == 0 ? $profit : "";

                                        $this->setCellRow($worksheet, $rowIndex, $tasteRow);
                                        $s++;
                                        $rowIndex++;
                                    }
                                }
                ...
                } else if (count($products["rows"] > 1)) {
                    if ($productKey == 0) {
                        if ($quantity > 1) {
                            for ($q2 = 1; $q2 <= $quantity; $q2++) {
                                if ($q2 == 1) {
                                    if (count($tasty) >= 1) {
                                        $s = 0;
                                        while($s < count($tasty)) {
                                ...
                                if ($q3 == 1) {
                                    if (count($tasty) >= 1) {
                                        ...
                                        while($s < count($tasty)) {
                                            ...
                                            $tasteRow[8] = $shippingDate;
                                            $tasteRow[9] = $s == 0 ? $product["name"] : "";
                                            $tasteRow[10] = (int)$tasty[($s)][1];//count($tasty) > 1 ? (int)$tasty[$s][1] : (int)$tasty[$s][0];//(int)$tasty[($s)][1];
                                            $tasteRow[11] = $tasty[$s][0];
                                            $tasteRow[12] = $s == 0 ? $newCartText : "";
                                            $tasteRow[13] = $s == 0 ? $paperCardText : "";
                                            $tasteRow[14] = "";
            ...
            $separatorRow[5] = "";
            $separatorRow[6] = "";
            $separatorRow[7] = $dateAdded;
            $separatorRow[8] = $shippingDate;
            $separatorRow[9] = "";
            $separatorRow[10] = "";
            $separatorRow[11] = "";
            $separatorRow[12] = "";
            ...
            $this -> setCellRow($worksheet, $rowIndex, $separatorRow, $box_format);
            $worksheet -> getStyle("A".($rowIndex).":"."V".($rowIndex)) -> applyFromArray (
                array (
                    "fill" => array (
                        "type" => PHPExcel_Style_Fill::FILL_SOLID,
                        "color" => array("rgb" => "C8DAE0")
                        )
                )
            );
            $worksheet->getStyle('G1:G9999')
                        ->getNumberFormat()->setFormatCode(
                            PHPExcel_Style_NumberFormat::FORMAT_DATE_XLSX16
                        );
                    $worksheet->getStyle('A1:A9999')
                        ->getNumberFormat()->setFormatCode(
                            PHPExcel_Style_NumberFormat::FORMAT_GENERAL
                        );
                    $worksheet->getStyle('H1:H9999')
                        ->getNumberFormat()->setFormatCode(
                            PHPExcel_Style_NumberFormat::FORMAT_DATE_XLSX16
                        );
                    $this->setColumnStyles( $worksheet, $styles, 2, $rowIndex );
            $rowIndex++;
        }
...
/*
    protected function populateOrdersWorksheet( &$worksheet, &$languages, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
        ...
        $sql = "SHOW COLUMNS FROM `".DB_PREFIX."order` LIKE 'meta_title'";
        $query = $this->db->query( $sql );
        $exist_meta_title = ($query->num_rows > 0) ? true : false;
        ...
        // Set the column widths
        $j = 0;
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(4);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(6);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(4);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(15);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(17);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(17);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(15);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(15);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(35);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(4);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('shipping_address_1'),19)+1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('custom_field'),19)+1);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('shipping_method'),16)+5);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('order_status_id'),16)+5);
        //$worksheet->getColumnDimensionByColumn($j)->setWrapText(true);
        $worksheet->getStyle('N'.$j.':O'.$j)->getAlignment()->setWrapText(true);


        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('date_modified'),16)+5);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('date_modified'),16)+5);
        $worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('date_modified'),16)+5);
        ...
        $styles = array();
        $data = array();
        $i = 1; //номер строки для старта тайтлов
        $j = 0; //номер ячейки для старта тайтла
        $data[$j++] = '№ заказа ';
        $data[$j++] = '№ заказа на сайте';
        $data[$j++] = 'Нужно ли подтверждение?';
        $data[$j++] = 'Имя клиента';
        $data[$j++] = 'Телефон';
        $data[$j++] = 'Мэйл';
        $data[$j++] = 'Дата создания';
        $data[$j++] = 'Дата получения';
        $data[$j++] = 'Наимнование товара';
        $data[$j++] = 'Количество пирожных';
        $data[$j++] = 'Вкус';
        $data[$j++] = 'Изменение надписи на открытке';
        $data[$j++] = 'Текст бумажной открытки';
        $data[$j++] = 'Комментарий к заказу';
        $data[$j++] = 'Доставка/самовывоз';
        $data[$j++] = 'Интервал доставки';
        $data[$j++] = 'Адрес доставки';
        $data[$j++] = 'Имя для доставки';
        $data[$j++] = 'Телефон для доставки';
        $data[$j++] = 'Откуда узнали';
        $data[$j++] = 'Курьер';
        $data[$j++] = 'Выручка';



        $worksheet->getRowDimension($i)->setRowHeight(90); //высота ряда
        $this->setCellRow( $worksheet, $i, $data, $box_format ); // добавляет row с тайтлами в ексель
        ...
        $orders = $this->getOrders( $languages, $exist_meta_title, $offset, $rows, $min_id, $max_id );

        $len = count($orders);
        $min_id = $orders[0]['order_id'];
        $max_id = $orders[$len-1]['order_id'];
        ...
        foreach ($orders as $row) {
            ...
            $order_products = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . $row['order_id'] . "'");
...
///START-------
            ...
            if (isset($order_products['row']['quantity']) && intval($order_products['row']['quantity']) > 1) {
                $order_products['row']['quantity'] = 1;
            }

            for ($w = 0; $w < count($order_products['rows']); $w++) {
                    if (intval($order_products['rows'][$w]['quantity']) !== 1) {
                        $quantity = $order_products['rows'][$w]['quantity'];
                        $quan_name= $order_products['rows'][$w]['name'];
                        $order_products['rows'][$w]['quantity'] = 1;
                    }
                }
//                echo $quantity;
//                echo $quan_name;
...
//
//                var_dump($order_products["rows"]);
//            die;

                foreach ($order_products["rows"] as $product) {

                    $order_coupon = $this->db->query("SELECT title FROM " . DB_PREFIX . "order_total WHERE order_id = '" . $row['order_id'] . "'");
                    $order_coupon = (array)$order_coupon;

                    $where_about = $this->db->query("SELECT custom_field FROM " . DB_PREFIX . "customer WHERE email = '" . $row['email'] . "'");
                    $where_about = (array)$where_about;

                    $order_tastes = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . $row['order_id'] . "' AND order_product_id = '".$product["order_product_id"] . "'");
                    $tastes=(array)$order_tastes;// все вкусы для заказа
//                    if($row['order_id'] == 1024){
//                        var_dump($order_products["rows"]);
////                        var_dump($tastes);
//                        die;
//                    }

                    $order_product_id = $this->db->query("SELECT order_product_id FROM " . DB_PREFIX . "order_product WHERE order_id = '" . $row['order_id'] . "'");
                    $order_product_id=(array)$order_product_id;
                    $order_product_id=$order_product_id['row']['order_product_id'];
                    ...
                    $product_count = $this->db->query("SELECT isbn, upc FROM " . DB_PREFIX . "product WHERE product_id = '" . $prod_id . "'");
                    $product_count=(array)$product_count;
                    if($product_count['row']['upc']) {
                        $text_to_cart_add = $product_count['row']['upc']; //количество пирожных в 1 коробке
                    } else {
                        $text_to_cart_add = '';
                    }

                    $product_count=$product_count['row']["isbn"]; //количество пирожных в 1 коробке
...
//
                    if (count($tastes['rows']) >= 2) {

                        for ($u = 0; $u < count($tastes['rows']); $u++) {

                            if (trim($tastes['rows'][$u]['name']) == "Приложить открытку") {
                                $cart_with = "";
                                if (strpos($tastes['rows'][$u]['value'], "@%")) {
                                    $cart_with = explode("@%", $tastes['rows'][$u]['value']);
                                }
                            }

                            if ($tastes['rows'][$u]['name'] == "Введите имя") {
                                $new_cart_text = $text_to_cart_add.' '.(string)$tastes['rows'][$u]['value'];
                            }

                            if ($tastes['rows'][$u]['name'] == "Введите надпись ") {
                                $new_cart_text = (string)$tastes['rows'][$u]['value'];
                            }

                            if ($tastes['rows'][$u]['name'] == "Текст открытки") {
                                $cart_with = (string)$tastes['rows'][$u]['value'];
                            }

                            if ($tastes['rows'][$u]['name'] == "Вкус количество") {

                                if (trim($tastes['rows'][$u]['value']) == "@%") {

                                    //if (isset($tastes['rows'][$u-1]['value'])) {
                                    //ToDo Возможно вкус не только в 0 ячейке
                                    if (isset($tastes['rows'][0]['value'])) {

                                        if ($tastes['rows'][$u]['order_product_id'] == $tastes['rows'][0]['order_product_id']) {

                                            $tasty[0][0] = $tastes['rows'][0]['value'];
                                            if ($tastes['rows'][$u]['name'] !== 'Текст открытки' && $tastes['rows'][$u]['name'] !== 'Дополнительно:') {
                                                if ($tastes['rows'][$u]['value'] == '@%') {

                                                    for($re = 0; $re < count($tastes['rows']); $re++) {
                                                        if ($tastes['rows'][$re]['name'] == "Вкус единичный") {
                                                            $tasty[0][0]=$tastes['rows'][$re]['value'];
                                                        }
                                                    }
//                                                $tasty[0][0]='ассорти';
                                                } else {
                                                    $tasty[0][0] = $tastes['rows'][$u]['value'];
                                                }
                                            }
                            ...
                            } elseif (strpos($tastes['rows'][$u]['value'], ";")) {

                                $taste = explode(";", $tastes['rows'][$u]['value']);
                                $tasty1 = array_pop($taste);
                            ...
                            } elseif((trim($tastes['rows'][$u]['value']) === "@%") && (count($tastes['rows']) == 2) && (isset( $tastes['rows'][$u+1]))){
                                $tasty[0][0] = $tastes['rows'][$u-1]['value'];
                ...
                } elseif (count($tastes['rows']) == 1){


                    for ($u = 0; $u < count($tastes['rows']); $u++) {
                        if ($tastes['rows'][$u]['name'] == "Вкус количество") {
                            if (trim($tastes['rows'][$u]['value']) == ";@%") {
                                if ($tastes['rows'][$u]['order_product_id'] == $tastes['rows'][$u - 1]['order_product_id']) {
                                    $tasty[0][0] = $tastes['rows'][$u - 1]['value'];
                                } else {
                                    $tasty[0][0] = " ";
                                }
                                ...
                                if($tastes['rows'][$u]['name'] !== "Вкус единичный") {
                                    $tasty[0][0] = "ассорти";
                                    $tasty[0][1] = $product_count;
                                }
                            }
...
// ------------------END---------------------
                ...
                $shipping_row = explode("\n",htmlspecialchars($row["shipping_address_1"]));


                if ($shipping_row[0] === " ") {
                    $addrText ="";
                }else{
                    $addrText = $shipping_row[0];
                }



                if (isset($shipping_row[2])) {
                    if ($shipping_row[2] === " ") {
                        $date_for =" ";
                    } else {
                        $date_for = $shipping_row[2];
                    }
                }
...
//                            }
                ...
                if ($order_products['row']['name'] == $product['name']){

                //if ($order_products['row']['product_id'] == $product['id']){

                    if (!in_array($product['order_id'],$order_array))  {
                        $data[$j++] = "";  //nomer zakaza
                        $data[$j++] = $row['order_id'];  //nomer zakaza
                        $data[$j++] = $ansConfirm; // podtverjdenie
                        $data[$j++] = $row['firstname'];// imya
                        $data[$j++] = $row['telephone']; // telefon
                        $data[$j++] = $row['email'];    // mail
                    ...
                    $data[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($row['date_added'], 0, 10))),date('m',strtotime(substr($row['date_added'], 0, 10))),date('d',strtotime(substr($row['date_added'], 0, 10))));
                    if(isset($date_for)){
                        $qwee = date("d.M", strtotime(trim($date_for))); //data izmeneniya
                        if ($qwee == '01.01.1970'){
                            $data[$j++]=" ";
                        }else{
                            $data[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(trim($date_for))),date('m',strtotime(trim($date_for))),date('d',strtotime(trim($date_for))));
                        }

                    }else{
                        $data[$j++] =" ";
                    }
                    //array_push($order_array, $product['order_id']);
                }
...
//                        $data[$j++] = date("d.M", strtotime(trim($date_for))); //data izmeneniya
                        $data[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(trim($date_for))),date('m',strtotime(trim($date_for))),date('d',strtotime(trim($date_for))));
...
//                }
                ...
                if (isset($product['name'])) {
                    $data[$j++] =$product['name']; //nazvanie produkta
                }else{
                    $data[$j++] = " "; //nazvanie produkta
                }


                if (isset($tasty[0][1])) {
                    $data[$j++] = (int)$tasty[0][1]; //kolichestvo
                }else{
                    $data[$j++] = (int)$product_count; //kolichestvo
                }

                if (isset($tasty[0][0])) {
                    if ($product['product_id'] == 83 || $product['product_id'] == 84 || $product['product_id'] == 85){
                        //$data[$j++] ="м ". $tasty[0][0];
                        $data[$j++] = $tasty[0][0];
                    }else{
                        $data[$j++] = $tasty[0][0];
                ...
                if (isset($new_cart_text)) {
                    $data[$j++] = $new_cart_text;
                    $new_cart_text="";// izmenenie nadpisi na otkrytke
                }else{
                    $data[$j++] = " ";  // izmenenie nadpisi na otkrytke
                }

                if (isset($cart_with)) {
                    $data[$j++] = $cart_with;
                    $cart_with = "";//text bumajnoi okrytki
                } else {
                    $data[$j++] = ''; //text bumajnoi okrytki
                }


                if (!in_array($product['order_id'], $order_array)) {
                    if ($order_products['row']['name'] == $product['name']) {
                        $data[$j++] = $commentText; //text commentaria
                        $data[$j++] = $row['shipping_method'];// kuda dostavit'

                        if ($row['custom_field']) {
                            $inter = substr($row['custom_field'], strpos($row['custom_field'], '"'), 7);
                            if (strpos($row['custom_field'], "empty")) {
                                $data[$j++] = "";
                            } elseif (strpos($inter, "11-14")) {
                                $data[$j++] = '11-14';
                            } elseif (strpos($inter, "14-17")) {
                                ...
                                //$data[$j++] = "";
                                $data[$j++] = substr($row["custom_field"], 6, -2);
                            }

                        } else {
                            $data[$j++] = ""; //interval dostavki pustoi
                        }
                        $data[$j++] = $addrText;  //adres dostavki
                        if (isset($row['payment_firstname'])) {
                            $data[$j++] = $row['payment_firstname']; //nazvanie produkta
                        } else {
                            $data[$j++] = ""; //nazvanie produkta
                        }
                        if (isset($row['payment_lastname'])) {
                            $data[$j++] = " " . $row['payment_lastname']; //nazvanie produkta
                        } else {
                            $data[$j++] = ""; //nazvanie produkta
                        }
                        if (isset($where_about['row']['custom_field'])) {
                            $data[$j++] = $where_about['row']['custom_field']; //nazvanie produkta
                        } else {
                            $data[$j++] = ""; //nazvanie produkta
                        }
                        if (isset($coupon)) {
                            $data[$j++] = $coupon; //nazvanie produkta
                        } else {
                            $data[$j++] = ""; //nazvanie produkta
                        }
                        //adres dostavki
                        $data[$j++] = (int)$row['total']; //price
...
//                }
                ...
                if (count($tasty) > 1) {
                    ...
                    while($s < count($tasty)) {
                        //$s++;
                        $j = 0;
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($row['date_added'], 0, 10))),date('m',strtotime(substr($row['date_added'], 0, 10))),date('d',strtotime(substr($row['date_added'], 0, 10))));

                        $qwee = date("d.M", strtotime(trim($date_for))); //data izmeneniya
                        if ($qwee == '01.01.1970'){
                            $newdata[$j++]=" ";
                        } else {
                            $newdata[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(trim($date_for))),date('m',strtotime(trim($date_for))),date('d',strtotime(trim($date_for))));
                        }
//                        $newdata[$j++]= '';
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = (int)$tasty[($s)][1];  //количесто пирожных


                        if($product['product_id'] == 83 || $product['product_id'] == 84 || $product['product_id'] == 85) {
                            //$newdata[$j++] ="м". $tasty[$s][0];
                            $newdata[$j++] = $tasty[$s][0];
                        } else {
                            $newdata[$j++] = $tasty[$s][0];
                        }
                        ...
                        $newdata[$j++] =  '';//creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = $row['shipping_method'];// kuda dostavit'
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        $newdata[$j++] = ""; //creation date
                        ...
                        $this->setCellRow($worksheet, $i+$s, $newdata);
...
// white row //
            ...
            $whiterow[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(substr($row['date_added'], 0, 10))),date('m',strtotime(substr($row['date_added'], 0, 10))),date('d',strtotime(substr($row['date_added'], 0, 10))));


            $qwee = date('d.M', PHPExcel_Shared_Date::ExcelToPHP(strtotime(trim($date_for)))); //data izmeneniya
            if ($qwee == '01.01.1970'){
                $whiterow[$j++]=" ";
            }else{

                $whiterow[$j++] = PHPExcel_Shared_Date::FormattedPHPToExcel(date('Y',strtotime(trim($date_for))),date('m',strtotime(trim($date_for))),date('d',strtotime(trim($date_for))));
            ...
            $this->setCellRow($worksheet, $i, $whiterow);
            ...
            $ba = $i-1;
            $inte = "A".$ba.":"."V".$ba;
//            $worksheet->getStyle($inte)->applyFromArray(array(
//                'borders' => array(
//                    'top' => array(
...
//            ->getNumberFormat()
//            ->setFormatCode(
//                'd.M'  // my own personal preferred format that isn't predefined
//            );
            ->getNumberFormat()->setFormatCode(
                PHPExcel_Style_NumberFormat::FORMAT_DATE_XLSX16
            );
        ...
        try {
            ...
            switch ($export_type) {
                ...
                case 'or':
                    ...
                    $workbook->getActiveSheet()->getStyle('A1:V1')->getFont()->setBold(true)->setSize(9);
                    $workbook->getActiveSheet()->setAutoFilter('A1:V1');

//                    $workbook->getActiveSheet()->getStyle(
//                        'A1:' .
//                        $workbook->getActiveSheet()->getHighestColumn() .
//                        $workbook->getActiveSheet()->getHighestRow()
...
//                    ));
                    ...
                    $workbook->getActiveSheet()->getStyle("A1:V999")->applyFromArray(array(
                        'borders' => array(
                            'allborders' => array(
                                'style' => PHPExcel_Style_Border::BORDER_THIN
                            )
                    ...
                    $workbook->getActiveSheet()->getStyle('A1:V1')->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP)->setWrapText(true);
                    $workbook->setActiveSheetIndex($worksheet_index++);
                    $worksheet = $workbook->getActiveSheet();
                    $worksheet->setTitle( 'Заказы' );

                    $this->populateOrdersWorksheet( $worksheet, $languages, $box_format, $text_format, $offset, $rows, $min_id, $max_id );
                    $worksheet->freezePaneByColumnAndRow( 1, 2 );
                    break;
